...
import { getEndUserProfile } from "@/utils/end-user/purchase-requests"
...
export default function EndUserProfilePage() {
  ...
  useEffect(() => {
    async function loadEndUserProfile() {
      try {
        const profile = await getEndUserProfile()
        setUser(profile)
      } catch (error) {
        console.error("Error loading end-user profile:", error)
        setError("Failed to load end-user profile. Please try again later.")
      } finally {
        setIsLoading(false)
      }
    }
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-gray-500 text-center">
          ...
          <p>Unable to retrieve end-user profile information.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl font-bold text-center text-[#2E8B57]">End-User Profile</CardTitle>
          </CardHeader>
