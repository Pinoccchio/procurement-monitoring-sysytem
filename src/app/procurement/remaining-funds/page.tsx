...
import { getUserProfile } from "@/utils/procurement/purchase-requests"
import type { User } from "@/types/procurement/user"
import { supabase } from "@/utils/auth"
import type { RealtimeChannel } from "@supabase/supabase-js"

export default function ProcurementRemainingFundsPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const [profile, funds] = await Promise.all([getUserProfile(), getRemainingFunds()])
        setUser(profile)
  ...
  //Removed handleUpdateFunds function

  return (
    <div className="container mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6 text-[#2E8B57]">Procurement Remaining Funds View</h1>
