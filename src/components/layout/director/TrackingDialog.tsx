...
import { updatePurchaseRequestStatus, getTrackingHistory } from "@/utils/director/purchase-requests"
...
export function TrackingDialog({ isOpen, onClose, purchaseRequest, onUpdate }: TrackingDialogProps) {
  ...
  const handleUpdate = async () => {
    setIsUpdating(true)
    try {
      let message = `Purchase request ${status} by Director: ${remarks}`
      if (status === "forwarded" || status === "returned") {
        message = `Purchase request ${status} to ${nextDesignation} by Director: ${remarks}`
      }
  ...
  const canChangeStatus = () => {
    return purchaseRequest.current_designation === "director" && purchaseRequest.status !== "forwarded"
  }
              ...
              {(status === "forwarded" || status === "returned") && (
                <div className="space-y-2">
                  ...
                  <Select value={nextDesignation} onValueChange={(value: PRDesignation) => setNextDesignation(value)}>
                    ...
                    <SelectContent>
                      {["procurement", "admin", "budget", "bac", "supply"].map((designation) => (
