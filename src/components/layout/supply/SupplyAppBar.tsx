...
import { getTrackingHistory, supabaseClient } from "@/utils/supply/purchase-requests"
import type { TrackingEntry } from "@/types/procurement/purchase-request"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { signOut } from "@/utils/auth"

interface SupplyAppBarProps {
  isSidebarOpen: boolean
  onSidebarOpenChange: (open: boolean) => void
  className?: string
}

export function SupplyAppBar({ isSidebarOpen, onSidebarOpenChange, className }: SupplyAppBarProps) {
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const filteredHistory = history.filter((entry) => entry.notification_type === "supply")
    ...
    const subscription = supabaseClient
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: "notification_type=eq.supply",
        },
        (payload) => {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-[#2E8B57] hidden sm:block">Supply Management</h1>
          </div>
                            ...
                            <div className="flex items-start gap-3">
                              <div className="flex-1 space-y-1">
                                ...
                                <p className="text-sm text-gray-500">
                                  {notification.status === "assessed"
                                    ? "Delivery inspected and matches the order"
                                    : notification.status === "discrepancy"
                                      ? "Delivery does not match the order"
                                      : notification.notes}
                                </p>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem asChild>
                  <Link href="/supply/profile">Profile</Link>
                </DropdownMenuItem>
