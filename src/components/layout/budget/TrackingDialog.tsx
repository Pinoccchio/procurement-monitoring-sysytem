...
import { updatePurchaseRequestStatus, getTrackingHistory } from "@/utils/budget/purchase-requests"
...
export function TrackingDialog({ isOpen, onClose, purchaseRequest, onUpdate }: TrackingDialogProps) {
  ...
  const handleUpdate = async () => {
    setIsUpdating(true)
    try {
      let message = `Purchase request ${status} by Budget: ${remarks}`
      if (status === "forwarded" && nextDesignation === "procurement") {
        message = `Purchase request forwarded to procurement for Quotation and Abstract of Prices by Budget: ${remarks}`
      } else if (status === "forwarded" || status === "returned") {
        message = `Purchase request ${status} to ${nextDesignation} by Budget: ${remarks}`
      }
    ...
    switch (entry.status) {
      case "forwarded":
        if (entry.designation === "procurement") {
          if (entry.notes && entry.notes.includes("quotation_and_abstract")) {
            return (
              <span className={statusClass}>
                Purchase request forwarded to procurement for Quotation and Abstract of Prices by Budget
              </span>
            )
          }
        }
  ...
  const canChangeStatus = () => {
    return purchaseRequest.current_designation === "budget" && purchaseRequest.status !== "forwarded"
  }
              ...
              {(status === "forwarded" || status === "returned") && (
                <div className="space-y-2">
                  ...
                  <Select value={nextDesignation} onValueChange={(value: PRDesignation) => setNextDesignation(value)}>
                    ...
                    <SelectContent>
                      {["procurement", "admin", "director", "bac", "supply"].map((designation) => (
              ...
              {status === "forwarded" && nextDesignation === "procurement" && (
                <div className="space-y-2">
                  ...
                  <Select value={purchaseOrderType} onValueChange={(value: string) => setPurchaseOrderType(value)}>
                    ...
                    <SelectContent>
                      <SelectItem value="quotation_and_abstract">Quotation and Abstract of Prices</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              )}
