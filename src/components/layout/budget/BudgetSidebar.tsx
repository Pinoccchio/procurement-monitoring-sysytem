...
export function BudgetSidebar({ isOpen, onClose, className }: BudgetSidebarProps) {
        ...
        <div className="flex flex-col h-full">
          <div className="flex flex-col items-center gap-6 p-6 border-b bg-gradient-to-b from-white to-[#e6f3ed]">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-sm font-medium text-[#2E8B57]/80">Budget Management</p>
            </div>
          </div>

          <nav className="flex-1 space-y-1 p-4">
            <Link href="/budget/budget" onClick={onClose}>
              <Button
                variant="ghost"
                className={cn(
                  "w-full justify-start hover:text-white hover:bg-[#2E8B57] transition-colors duration-300",
                  pathname === "/budget/budget" ? "bg-[#2E8B57] text-white" : "text-[#2E8B57]",
                )}
              >
