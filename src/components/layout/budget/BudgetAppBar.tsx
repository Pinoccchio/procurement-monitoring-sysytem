...
import { getTrackingHistory, supabaseClient } from "@/utils/budget/purchase-requests"
import type { TrackingEntry } from "@/types/procurement/purchase-request"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { signOut } from "@/utils/auth"

interface BudgetAppBarProps {
  isSidebarOpen: boolean
  onSidebarOpenChange: (open: boolean) => void
  className?: string
}

export function BudgetAppBar({ isSidebarOpen, onSidebarOpenChange, className }: BudgetAppBarProps) {
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const filteredHistory = history.filter((entry) => entry.notification_type === "budget")
    ...
    const subscription = supabaseClient
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: "notification_type=eq.budget",
        },
        (payload) => {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-[#2E8B57] hidden sm:block">Budget Management</h1>
          </div>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem asChild>
                  <Link href="/budget/profile">Profile</Link>
                </DropdownMenuItem>
