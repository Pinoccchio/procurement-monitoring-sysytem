...
import { getTrackingHistory, supabaseClient } from "@/utils/bac/purchase-requests"
import type { TrackingEntry } from "@/types/procurement/purchase-request"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { signOut } from "@/utils/auth"

interface BacAppBarProps {
  isSidebarOpen: boolean
  onSidebarOpenChange: (open: boolean) => void
  className?: string
}

export function BacAppBar({ isSidebarOpen, onSidebarOpenChange, className }: BacAppBarProps) {
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const filteredHistory = history.filter((entry) => entry.notification_type === "bac")
    ...
    const subscription = supabaseClient
      ...
      .on(
        "postgres_changes",
        {
          ...
          filter: "notification_type=eq.bac",
        },
        (payload) => {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-[#2E8B57] hidden sm:block">BAC Management</h1>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button className="bg-[#2E8B57] hover:bg-[#1a5235] text-white transition-colors duration-300 px-2 sm:px-4">
                  <span className="hidden sm:inline">BAC</span>
                  <ChevronDown className="h-4 w-4 sm:ml-2" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem asChild>
                  <Link href="/bac/profile">Profile</Link>
                </DropdownMenuItem>
